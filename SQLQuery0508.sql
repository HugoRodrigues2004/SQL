...
AND Preco BETWEEN 10 AND 100
AND Unidades IS NOT NULL;

SELECT f.Empresa AS Fornecedor, Contato AS Telefone
FROM Fornecedores
...
FROM Funcionarios
WHERE Cidade IN ('Tianguá', 'Viçosa', 'Ubajara', 'Ibiapina')
GROUP BY Cidade
ORDER BY MaiorSalario DESC;

SELECT Cidade, AVG(Salario) AS MediaSalario
FROM Funcionarios
...
SELECT d.NumPed,
       SUM((p.Preco * d.Qtde) - d.Desconto) AS ValorTotal
FROM DetalhesPed d
JOIN Produtos p ON d.CodProd = p.CodProd
GROUP BY d.NumPed
ORDER BY d.NumPed;

SELECT c.Descr, COUNT(*) AS QtdeProdutos
FROM Produtos p
...
SELECT f.Empresa, COUNT(*) AS QtdeProdutos
FROM Produtos p
JOIN Fornecedores f ON p.CodFor = f.CodFor
WHERE p.Preco > 5 AND p.Preco < 10
GROUP BY f.Empresa
ORDER BY QtdeProdutos DESC;

SELECT Cargo, SUM(Salario) AS Total
FROM Funcionarios
WHERE Cargo LIKE '%gerente%' OR Cargo LIKE '%coordenador%'
GROUP BY Cargo
ORDER BY Total DESC;

SELECT Pais, AVG(Salario) AS MediaSalario
FROM Funcionarios
GROUP BY Pais;
